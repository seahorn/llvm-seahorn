...
; RUN: %instcombine %s -mtriple xcore-xmos-elf -S | %FileCheck %s -check-prefixes=CHECK,WITHSTPCPY,CHECK-IPRINTF
; RUN: %instcombine %s -mtriple=i386-pc-windows-msvc -S | %FileCheck %s --check-prefixes=CHECK,NOSTPCPY
; RUN: %instcombine %s -mtriple=i386-mingw32 -S | %FileCheck %s --check-prefixes=CHECK,NOSTPCPY
; RUN: %instcombine %s -mtriple=armv7-none-linux-android16 -S | %FileCheck %s --check-prefixes=CHECK,NOSTPCPY
; RUN: %instcombine %s -mtriple=armv7-none-linux-android21 -S | %FileCheck %s --check-prefixes=CHECK,WITHSTPCPY
; RUN: %instcombine %s -mtriple=x86_64-scei-ps4 -S | %FileCheck %s --check-prefixes=CHECK,NOSTPCPY
