...
;
; RUN: %instcombine %s -S | %FileCheck %s --check-prefix=ALL --check-prefix=GENERIC
; RUN: %instcombine %s -mtriple i386-pc-linux -S | %FileCheck %s --check-prefix=ALL --check-prefix=TARGET
; RUN: %instcombine %s -mtriple=arm64-apple-ios9.0 -S | %FileCheck %s --check-prefix=ALL --check-prefix=TARGET
; RUN: %instcombine %s -mtriple=arm64-apple-tvos9.0 -S | %FileCheck %s --check-prefix=ALL --check-prefix=TARGET
; RUN: %instcombine %s -mtriple=thumbv7k-apple-watchos2.0 -S | %FileCheck %s --check-prefix=ALL --check-prefix=TARGET
; RUN: %instcombine %s -mtriple=x86_64-apple-macosx10.11 -S | %FileCheck %s --check-prefix=ALL --check-prefix=TARGET
; RUN: %instcombine %s -mtriple=x86_64-freebsd-gnu -S | %FileCheck %s --check-prefix=ALL --check-prefix=TARGET
