...
; RUN: %instcombine -S %s -mtriple=x86_64-apple-macosx10.9 | %FileCheck %s --check-prefix=CHECK-FLOAT-IN-VEC
; RUN: %instcombine -S %s -mtriple=arm-apple-ios7.0 | %FileCheck %s --check-prefixes=CHECK,CHECK-DOUBLE-ALIGN4
; RUN: %instcombine -S %s -mtriple=arm64-apple-ios7.0 | %FileCheck %s --check-prefixes=CHECK,CHECK-DOUBLE-ALIGN8
; RUN: %instcombine -S %s -mtriple=x86_64-apple-macosx10.8 | %FileCheck %s --check-prefixes=CHECK-NO-SINCOS,CHECK-NO-SINCOS-DOUBLE-ALIGN8
; RUN: %instcombine -S %s -mtriple=arm-apple-ios6.0 | %FileCheck %s --check-prefixes=CHECK-NO-SINCOS,CHECK-NO-SINCOS-DOUBLE-ALIGN4
; RUN: %instcombine -S %s -mtriple=x86_64-none-linux-gnu | %FileCheck %s --check-prefixes=CHECK-NO-SINCOS,CHECK-NO-SINCOS-DOUBLE-ALIGN8
