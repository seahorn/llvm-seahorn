...
; RUN: %instcombine %s -S -mtriple "i386-pc-linux" | %FileCheck %s --check-prefixes=CHECK,DOUBLE-4BYTE-ALIGN
; RUN: %instcombine %s -S -mtriple "i386-pc-win32" | %FileCheck %s --check-prefixes=CHECK,DOUBLE-8BYTE-ALIGN
; RUN: %instcombine %s -S -mtriple "x86_64-pc-win32" | %FileCheck %s --check-prefixes=CHECK,DOUBLE-8BYTE-ALIGN
; RUN: %instcombine %s -S -mtriple "i386-pc-mingw32" | %FileCheck %s --check-prefixes=CHECK,DOUBLE-8BYTE-ALIGN
; RUN: %instcombine %s -S -mtriple "x86_64-pc-mingw32" | %FileCheck %s --check-prefixes=CHECK,DOUBLE-8BYTE-ALIGN
; RUN: %instcombine %s -S -mtriple "sparc-sun-solaris" | %FileCheck %s --check-prefixes=CHECK,DOUBLE-8BYTE-ALIGN
; RUN: %instcombine %s -S -mtriple "x86_64-pc-win32" -enable-debugify 2>&1 | %FileCheck --check-prefix=DBG-VALID %s
; RUN: %instcombine %s -S -mtriple "x86_64-pc-win32" -enable-debugify 2>&1 --try-experimental-debuginfo-iterators | %FileCheck --check-prefix=DBG-VALID %s
