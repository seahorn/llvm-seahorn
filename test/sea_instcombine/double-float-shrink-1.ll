; RUN: %instcombine %s -S -mtriple x86_64-unknown-linux-gnu | %FileCheck %s --check-prefixes=CHECK,LINUX,ISC99
; RUN: %instcombine %s -S -mtriple x86_64-pc-win32 | %FileCheck %s --check-prefixes=CHECK,ISC99
; RUN: %instcombine %s -S -mtriple x86_64-pc-windows-msvc16 | %FileCheck %s --check-prefixes=CHECK,MS64,ISC89
; RUN: %instcombine %s -S -mtriple i386-pc-windows-msvc | %FileCheck %s --check-prefixes=CHECK,ISC99
; RUN: %instcombine %s -S -mtriple i686-pc-windows-msvc17 | %FileCheck %s --check-prefixes=CHECK,MS32,ISC89
