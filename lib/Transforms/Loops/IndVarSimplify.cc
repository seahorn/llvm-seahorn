...
class SeaIndVarSimplify : public LoopPass {
  LoopInfo                  *LI;
  ScalarEvolution           *SE;
  DominatorTree             *DT;
...
Pass *llvm_seahorn::createIndVarSimplifyPass() {
  return new llvm_seahorn::SeaIndVarSimplify();
}

static llvm::RegisterPass<llvm_seahorn::SeaIndVarSimplify>
    X ("sea-indvars", "SeaHorn Induction Variable Simplification");
...
///   bar((double)i);
///
void SeaIndVarSimplify::handleFloatingPointIV(Loop *L, PHINode *PN) {
...
void SeaIndVarSimplify::rewriteNonIntegerIVs(Loop *L) {
...
///
void SeaIndVarSimplify::simplifyAndExtend(Loop *L,
...
void SeaIndVarSimplify::sinkUnusedInvariants(Loop *L) {
