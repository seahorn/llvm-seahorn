...
tryToUnrollLoop(Loop *L, DominatorTree &DT, LoopInfo *LI, ScalarEvolution &SE,
                ...
                bool ForgetAllSCEV, std::optional<unsigned> ProvidedCount,
  ...
  // LLVM 18 folded ApproximateLoopSize into UnrollCostEstimator; canUnroll()
  // subsumes the duplicatable + valid-cost checks (NotDuplicatable is private).
  UnrollCostEstimator UCE(L, TTI, EphValues, UP.BEInsns);
  if (!UCE.canUnroll()) {
    LLVM_DEBUG(dbgs() << "  Not unrolling loop which is not duplicatable or"
  ...
  bool IsCountSetExplicitly = computeUnrollCount(
      ...
      MaxOrZero,
      TripMultiple, UCE, UP, PP, UseUpperBound);
  if (!UP.Count) {
...
Pass *llvm_seahorn::createSeaLoopUnrollPass(int OptLevel, bool ForgetAllSCEV,
  ...
  return new SeaLoopUnroll(
      OptLevel, ForgetAllSCEV, Count == -1 ? std::nullopt : std::optional<unsigned>(Count),
