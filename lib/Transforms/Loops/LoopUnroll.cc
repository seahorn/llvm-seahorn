...
TargetTransformInfo::UnrollingPreferences llvm::gatherUnrollingPreferences(
    ...
    std::optional<bool> UserUpperBound, std::optional<unsigned> UserFullUnrollMaxCount) {
...
static LoopUnrollResult tryToUnrollLoop(
    ...
    bool OnlyWhenForced, bool ForgetAllSCEV, std::optional<unsigned> ProvidedCount,
  ...
  if (PP.PeelCount) {
    ...
    ValueToValueMapTy VMap; // LLVM 17: peelLoop gained a trailing VMap out-param
...
public:
  ...
  LoopUnroll(int OptLevel = 2, bool OnlyWhenForced = false,
             bool ForgetAllSCEV = false, std::optional<unsigned> Threshold = std::nullopt,
             std::optional<unsigned> Count = std::nullopt,
             std::optional<bool> AllowPartial = std::nullopt, std::optional<bool> Runtime = std::nullopt,
...
// SEAHORN(llvm17): createSimpleLoopUnrollPass removed upstream (legacy loop
// unroll pass creators are gone in LLVM 17); no in-repo callers.
...
PreservedAnalyses LoopUnrollPass::run(Function &F,
  ...
  while (!Worklist.empty()) {
    ...
    LoopUnrollResult Result = tryToUnrollLoop(
        ...
        /*Threshold*/ std::nullopt, UnrollOpts.AllowPartial, UnrollOpts.AllowRuntime,
