...
Value *llvm_seahorn::InstCombiner::dyn_castFNegVal(Value *V, bool IgnoreZeroSign) const {
...
Instruction *llvm_seahorn::InstCombiner::foldOpWithConstantIntoOperand(BinaryOperator &I) {
...
static Value *CreateBinOpAsGiven(BinaryOperator &Inst, Value *LHS, Value *RHS,
                                 llvm_seahorn::InstCombiner::BuilderTy &B) {
...
Value *llvm_seahorn::InstCombiner::SimplifyVectorOp(BinaryOperator &Inst) {
...
static bool AddReachableCodeToWorklist(BasicBlock *BB, const DataLayout &DL,
                                       ...
                                       llvm_seahorn::InstCombineWorklist &ICWorklist,
                                       const TargetLibraryInfo *TLI) {
...
static bool
combineInstructionsOverFunction(Function &F, llvm_seahorn::InstCombineWorklist &Worklist,
  ...
  for (;;) {
    ...
    llvm_seahorn::InstCombiner IC(Worklist, Builder, F.optForMinSize(), ExpensiveCombines,
				  AA, AC, TLI, DT, DL, LI);
...
PreservedAnalyses llvm_seahorn::InstCombinePass::run(Function &F,
...
void llvm_seahorn::SeaInstructionCombiningPass::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesCFG();
...
bool llvm_seahorn::SeaInstructionCombiningPass::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
...
// INITIALIZE_PASS_BEGIN(llvm_seahorn::SeaInstructionCombiningPass, "sea-instcombine",
...
// INITIALIZE_PASS_END(llvm_seahorn::SeaInstructionCombiningPass, "sea-instcombine",
//                     "Combine redundant instructions", false, false)
...
FunctionPass *llvm_seahorn::createInstructionCombiningPass(bool ExpensiveCombines) {
