...
Value *llvm_seahorn::InstCombiner::simplifyRangeCheck(ICmpInst *Cmp0, ICmpInst *Cmp1,
  ...
  if (AvoidUnsignedICmp) return nullptr;
...
Value *llvm_seahorn::InstCombiner::foldAndOfFCmps(FCmpInst *LHS, FCmpInst *RHS) {
...
static Instruction *matchDeMorgansLaws(BinaryOperator &I,
  ...
  if (match(I.getOperand(0), m_OneUse(m_Not(m_Value(A)))) &&
      ...
      !llvm_seahorn::IsFreeToInvert(A, A->hasOneUse()) &&
      !llvm_seahorn::IsFreeToInvert(B, B->hasOneUse())) {
...
Instruction *llvm_seahorn::InstCombiner::MatchBSwap(BinaryOperator &I) {
  Value *Op0 = I.getOperand(0), *Op1 = I.getOperand(1);
...
static Value *getSelectCondition(Value *A, Value *B,
  ...
  if (match(A, m_SExt(m_Value(Cond))) &&
    ...
    NotB = llvm_seahorn::peekThroughBitcast(NotB, true);
...
static Value *matchSelectFromAndOr(Value *A, Value *C, Value *B, Value *D,
  ...
  A = llvm_seahorn::peekThroughBitcast(A, true);
  B = llvm_seahorn::peekThroughBitcast(B, true);
...
Value *llvm_seahorn::InstCombiner::foldOrOfICmps(ICmpInst *LHS, ICmpInst *RHS,
  ...
  if (!AvoidBv && (PredL == ICmpInst::ICMP_ULT || PredL == ICmpInst::ICMP_ULE) &&
...
/// Optimize (fcmp)|(fcmp).  NOTE: Unlike the rest of instcombine, this returns
/// a Value which should already be inserted into the function.
Value *llvm_seahorn::InstCombiner::foldOrOfFCmps(FCmpInst *LHS, FCmpInst *RHS) {
  Value *Op0LHS = LHS->getOperand(0), *Op0RHS = LHS->getOperand(1);
  Value *Op1LHS = RHS->getOperand(0), *Op1RHS = RHS->getOperand(1);
  FCmpInst::Predicate Op0CC = LHS->getPredicate(), Op1CC = RHS->getPredicate();
