...
Instruction *llvm_seahorn::InstCombiner::OptAndOp(BinaryOperator *Op, ConstantInt *OpRHS,
                                    ConstantInt *AndRHS,
...
Value *llvm_seahorn::InstCombiner::foldAndOrOfICmpsOfAndWithPow2(ICmpInst *LHS, ICmpInst *RHS,
  ...
  if (match(LHS->getOperand(0), m_And(m_Value(A), m_Value(B))) &&
    ...
    if (A == C && isKnownToBeAPowerOfTwo(B, false, 0, &CxtI) &&
...
Instruction *llvm_seahorn::InstCombiner::visitAnd(BinaryOperator &I) {
  ...
  if (match(Op1, m_APInt(C))) {
    ...
    if (match(Op0, m_OneUse(m_Or(m_Value(X), m_APInt(OrC))))) {
      ...
      Value *And =
          Builder.CreateAnd(X, ConstantInt::get(I.getType(), Together ^ *C));
      And->takeName(Op0);
      return BinaryOperator::CreateOr(And,
                                      ConstantInt::get(I.getType(), Together));
    }
